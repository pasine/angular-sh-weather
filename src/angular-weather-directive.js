    ...
    var service = {
      ...
      getWeather: function (location, units) {
        ...
        $http.get('http://api.openweathermap.org/data/2.5/weather?q=' +
            location + '&units=' + units + '&cnt=5')
          .success(function (data) {
            if (data) {
              if (data.main) {
                service.curWeather[location].temp.current = data.main
                  .temp;
                service.curWeather[location].temp.min = data.main.temp_min;
                service.curWeather[location].temp.max = data.main.temp_max;
              }
              service.curWeather[location].clouds = data.clouds ?
                data.clouds
                .all : undefined;
            }
          });
  ...
  weatherModule.directive('todaysWeather', function (weatherService) {
    return {
      ...
      link: function (scope, iElem, iAttr) {
        ...
        scope.weather = weatherService.getWeather(scope.location, scope
          .units);
      }
    };
  });
